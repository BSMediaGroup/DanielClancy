...
export function buildClientContext(request, event = {}) {
  ...
  const url = (() => {
    try {
      return new URL(request?.url || "https://danielclancy.net/");
    } catch {
      return new URL("https://danielclancy.net/");
    }
  })();
  ...
  return {
    ...
    surface: cleanText(event.surface || "danielclancy_public", 120),
    domain: cleanText(event.domain || "danielclancy.net", 120),
    host,
    origin,
...
export async function postDanielClancyAlert(context, event) {
  ...
  const payload = {
    ...
    source_namespace: "danielclancy",
    project: "danielclancy",
    surface: cleanText(event.surface || clientContext.surface || "danielclancy_public", 120),
    domain: cleanText(event.domain || event.host || clientContext.domain || "danielclancy.net", 120),
