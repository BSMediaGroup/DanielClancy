...
export function validatePublicCartItems(products, cartItems = [], overrides = []) {
  const bundle = productOverrideBundle(overrides);
  const productMap = new Map();
  publicProducts(products, bundle.items, bundle.settings).forEach((product) => {
  ...
  for (const raw of Array.isArray(cartItems) ? cartItems : []) {
    ...
    const currency = cleanText(variant.currency || product.priceRange?.currency || BASE_STORE_CURRENCY, 12).toUpperCase();
...
async function hydrateListProductDetails(env, storeId, products) {
  ...
  const workers = Array.from({ length: Math.min(4, queue.length) }, async () => {
    while (queue.length) {
      ...
      if (detail.ok) {
        byId.set(product.printfulProductId, normalizePrintfulProduct(product.raw, detail.payload?.result));
      }
    }
  });
...
function categoryLabel(value) {
  if (value && typeof value === "object") {
    return cleanText(value.name || value.title || value.label || value.slug || value.id, 160);
  }
